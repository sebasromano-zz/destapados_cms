...
class NewsletterlistsController extends AppController {
	...
	function admin_edit( $id = false ) {
		...
		if ( !empty($this->data) ) {
			...
			if($this->Newsletterlist->save($this->data)){
				$this->redirect( array('action' => 'index') );
			}

        } else if ( !empty($id) ) {
			$this->request->data = $this->Newsletterlist->read(null, $id);
  		}
    ...
    function admin_delete( $id = false ) {
    	...
    	$this->Session->setFlash('<a class="close" data-dismiss="alert" href="#">×</a> '. __d( 'newster', 'Item(s) eliminado con éxito'), 'default', array( 'class' => 'alert alert-success') );

		if(!empty($id)){
			$this->Newsletterlist->delete($id);
		} else {
